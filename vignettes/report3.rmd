...
***
#### *Randy Posada*, Luna L. Sanchez-Reyes, Emily Jane McTavish
#### McTavish Lab
...
```{r Assigning_Chiroptera_Families,message=FALSE}
...
Retrieving subtree form the Open Tree Of Life.
...
Getting the dates available for the five taxa tree.

```{r}
YOUR_DATELIFE_RESULT_OBJECT <- datelife::get_datelife_result(my_tree)
```
...
`Plot Phylo` plots the output of `summarize_datelife_results`.The output is all the chronograms that have at least two of the taxa that are given as input to get the datelife result.
...
Getting a full Chiroptera Tree from the Open Tree of Life.
```{r}
