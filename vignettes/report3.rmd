...
`Plot Phylo` plots the output of summarize datelife reslts.The output is all the chronogram that have at least two of the taxa that are given as input to get datelife result.
```{r}
datelife::plot_phylo_all(trees = chiroptera_phylo_all)
```
In order to for the function `plot_phylo_all` the package `phyloch` needs to be installed.
