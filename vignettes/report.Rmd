...
It is relevant that our taxon is monophyletic since nonmonophyletic taxon contain 'invalid' or 'broken' data. When the taxon is 'broken', its ott_id is not assidigned to a node in the synthetic tree.
```{r}
rotl::is_in_tree(chiroptera_ott_id)
```
the Above code confirmed that indeed our taxon is monophyletic by giviing the output 'TRUE'

WHY DO WE NEED THE NODE OF THE TAXON  #ask luna if okay 
Ott_ids and Node ids allow us to interact with the synthetic OTOL.
```{r Getting_Node_info}
