...
```{r}
chiroptera_subtree <- rotl::tol_subtree(ott_id = resolved_names["chiroptera",]$574724)

```

```{r}
subtree <- rotl::tol_subtree(resolved_names["chiroptera",]$ott_id)
```

```{r}
rotl::is_in_tree(resolved_names["chiroptera",]$ott_id)

```

```{r}
chiroptera_node_info <- rotl::tol_node_info(resolved_names["Chiroptera",]$ott_id)
...
```{r}
chiroptera_families <- datelife::get_ott_children(ott_ids = resolved_names["Chiroptera",]$ott_id, ott_rank = "family")
